# CLAUDE.md – TiNi 3D Admin Dashboard

> Companion admin panel for **TiNi 3D Store** (`../3dprinting_ui`).
...
| Field | Value |
|---|---|
| Project | TiNi 3D Admin Dashboard |
...
| Auth | Admin-only: `admin` / `admin123` → localStorage key `tini-admin-auth` |
...
| `/orders` | Orders | Manage orders from localStorage `tini-orders`, update status |
...
- **Refresh token** — `localStorage` key `tini-admin-refresh-token` (7-day TTL, rotated on use)
- Legacy key `tini-admin-auth` kept for name display only (not security-relevant)
...
| Orders | `localStorage: tini-orders` (shared với storefront) | Status update + delete localStorage | ⚠️ Mock — chờ BE **P1-6** |
